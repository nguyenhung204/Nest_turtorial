...
export class AuthService {
  ...
  async register(createUserDto: CreateUserDto) {
    ...
    const refreshToken = this.jwtService.sign({userName : savedUser.username, _id : savedUser._id});
  ...
  async validateUser({username, password }: AuthPayloadDto ): Promise<any> {
    ...
    if ( await bcrypt.compare(password, user.password)) {
      ...
      refresh_token: this.jwtService.sign(payLoad, {secret : jwtConstants.secret,expiresIn: '1h'})
    };
    }
    else {
...
async validateUserFacebook(userFacebook : AuthSocialDto) {
  ...
  const user = await this.userModel.findOne({ email: userFacebook.email });
  console.log(user || "User not found");
  if(user) return user;
  console.log('User not found! Creating new user...');
